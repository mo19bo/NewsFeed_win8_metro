...
namespace App21
{
    ...
    public class FeedDataSource
    {
        ...
        public async Task GetFeedsAsync()
        {
            ...
            StorageFile file = await root.CreateFileAsync("URL.txt", CreationCollisionOption.OpenIfExists);
            
         
            IList<string> x = await FileIO.ReadLinesAsync(file);
            x.Add("http://tech.163.com/special/000944OI/headlines.xml");
                foreach (var s in x)
                {
                    Task<FeedData> feed =
                        GetFeedAsync(s);
